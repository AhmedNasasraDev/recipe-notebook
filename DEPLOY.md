...
- **רענון אותו Preview — עיצוב ההגדרות, שלב 3 של ביקורת העיצוב** (23.09.2026): פריסה מחדש, שוב בלי `target`, אחרי פיצול `/settings` לרשימת קטגוריות (`פרטים אישיים`, `שפה`, `העדפות מתכונים`, `פרטיות ואבטחה`, `גיבוי ונתונים`) ומיזוג שני כרטיסי השם והתמונה למסך `/settings/profile` לאחד. הכתובת היציבה של הענף נשארת אותו דבר; כתובת הפריסה הספציפית הזו: `https://recipe-notebook-preview-1rsm08oi5-ahmad-nsasra.vercel.app` (`dpl_5UtdtGXB1TLHeu6WKCkEHqVEFa7z`, קומיט `3c9c79b`).
