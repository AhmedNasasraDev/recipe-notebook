...
- **רענון ה־Preview** (23.09.2026): נפרסה מחדש כל עבודת מסמך האפיון (שלבים 0–7 + ארבעת תיקוני ההמשך — תפריט ⋮, Hero, לוח צבעים 3.1, Frank Ruhl Libre, אזורי לחיצה 44px, ניגודיות אפור/ענבר, כפתורי הדפסה). פריסה `dpl_EaqTdTcQPwFnHBwRL83g9DCJtdkZ` מהקומיט `0436f00` (הענף `claude/recipe-notebook-full-system-qa-fb7p1y`), `READY`. הכתובת הקבועה `https://recipe-notebook-preview-ahmad-nsasra.vercel.app` מצביעה עליה עכשיו.
