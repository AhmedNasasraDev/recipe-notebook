...
- **מיגרציה 0040** (23.09.2026, אפיון שלב 3א, A-11): policy הקריאה ב־`storage.objects` מאפשרת גם למי שהעלה קובץ לראות אותו אחרי שהמתכון נמחק, כדי ש־`remove()` של Storage ימצא וימחק אותו — הוחלה על הפרויקט באישור. ה־Preview עדיין מהקומיט `f4b807d` (לא נפרס מחדש בשלב זה).
