...
| `demo.html` + `viewer/demo.tsx` | the **SHAREABLE DEMO**: the same product screens and the same route table, with sample data that is remembered in `localStorage` and a notice that says so. Built to one file by `npm run demo`. |
| `viewer/routes.tsx` | the route table both builds render — the one copy `scripts/check-routes.mjs` compares with `App.tsx`. |
| `viewer/demoRepository.ts` · `viewer/demoStore.ts` | the fixture, wrapped so the demo remembers what a person made in it; `localStorage`, measured rather than assumed. |
| `viewer/DemoNotice.tsx` | what the demo says about itself: sample data, local saving, no account, no sync. |
| `scripts/standalone.mjs` | folds the demo build into ONE html file, and refuses to pack one that carries a key, a Supabase URL or the owner's name. |
| `scripts/demo-check.mjs` | drives the PACKED FILE over `file://` — 42 checks, including a browser that refuses to store anything and the same file served from a URL. |
...
npm run demo                                          # → artifact/dist-demo/recipe-notebook-demo.html
npm run demo:check                                    # 42 checks on that packed file, over file://
